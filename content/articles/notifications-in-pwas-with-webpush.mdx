...
The next step would be to ask for permission from the user to display notifications. The `Notification.requestPermission()` recently changed from using callbacks to promises that is why we cover both callbacks and promises in the example below (we never know which version of the api a browser implemented). 
...
After registering our service worker and getting user permission we can now subscribe the user to the Push API by calling `registration.pushManager.subscribe()` 
...
2. Within the `registration.pushManager.subscribe()` function we are passing an object with two properties
    1. **userVisibleOnly** - A boolean that must be set to `true` indicating that the user will be notified every time a push message is sent. Silent messages are not allowed.
    2. **applicationServerKey -** This is a public key generated in a pair along with a private key and are unique to your app. Application server keys otherwise knowns as VAPID keys are used by a push service to identify the application subscribing a user and ensure that the same application is messaging that user. 
...
- It should also be noted that the `Notification.requestPermission()` is available only in secure contexts (HTTPS).
