...
[Stripe](https://stripe.com/) is a payment infrastructure that allows you to accept payments and send payouts. Its easiness of setting up is one of the factors that has made it a favorite among developers and even folks without any technical experience.

## About the Timeship

The [Timeship](https://timeship.codedays.co/) is a project that allows you to “Time Travel”. Pretty cool if you ask me 😌. The user enters a location, a year and the Timeship takes them there.
...
A checkout session is what the customer will see as they pay for their ticket through the Stripe checkout page we'll redirect them to. Below is a serverless function that accepts a POST request with customer details and returns a checkout url.
...
First, let’s create an endpoint that Stripe will POST to immediately payment is complete.
...
The endpoint receives the `type` property in the body of the request which identifies the type of event. We specifically want the `checkout.session.completed` event. After confirming the type of event, using the `data.object.id` we can check the payment status.

To prevent tickets from being re-used, I save all completed session ids to a base on [airtable](https://airtable.com/) and use that to cross-check with the session id of the incoming request.
...
export default async function handler(req, res){
  if(req.method === "POST"){
    try{
      ...
      //Check if ticket is being re-used 👮🏽‍♂️
      ...
      const foundTicket = records.find((passenger) => passenger.fields.ticket === stripeSession.id);
      if(foundTicket) {
...
If a session id has made it this far then we add it to airtable alongside the metadata we sent to Stripe in Step 4.
...
For Stripe to make requests to our endpoint we need to setup a webhook on the Stripe dashboard.
